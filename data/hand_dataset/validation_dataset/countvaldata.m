function countvaldata
%This function gives the statistics of the validation data
datasets = {'Movie'};
datasets_fullname = {'Movie dataset'};
count = zeros(1,1);
countimg = zeros(1,1);
countbig = zeros(1,1);
uf = dir('validation_data/annotations/*.mat');
for i = 1:length(uf)
    ...
    I = find(index_cmp > 0);
    flag = 0;
    im = imread(sprintf('validation_data/images/%s.jpg',imname));
    load(sprintf('validation_data/annotations/%s',uf(i).name));
    [h w d] = size(im);
