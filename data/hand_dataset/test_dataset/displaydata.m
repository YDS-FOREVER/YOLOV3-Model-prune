...
% This function displays test data with annotations overlaid. Other
% datasets could be seen in the same way by changing the paths of image
% and annotation directories

uf = dir('test_data/images/*.jpg');
for i = 1:length(uf)
    dot = strfind(uf(i).name,'.');
    imname = uf(i).name(1:dot-1);
    load(['test_data/annotations/' imname '.mat']);
    im = imread(['test_data/images/' uf(i).name]);
    imshow(im);
